...
** volá funkci Error().
**/
    tElemPtr prvy = L->First;
    L->First = (tElemPtr) malloc(sizeof(tElemPtr));
    if(L->First == NULL){
        Error();
    }
    L->First->ptr = prvy;
...
** nic se nedìje.
**/
    tElemPtr prvok; // prvok ktory chceme rusit
...
**/
    ...
    tElemPtr prvok = L->Act->ptr; // prvok za aktivnym prvkom

    L->Act->ptr = (tElemPtr) malloc(sizeof(tElemPtr));
    if(L->Act->ptr == NULL){
        Error();
    }
