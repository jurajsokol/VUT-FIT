...
**/
	...
	if((*RootPtr) == NULL){ // vnorim sa do praveho podstromu
		(*RootPtr) = PtrReplaced->LPtr;
		ReplaceByRightmost(PtrReplaced, &PtrReplaced->LPtr);
	}
	else{
		if((*RootPtr)->RPtr != NULL){ // hlada najpravejsi uzol laveho podstromu
			ReplaceByRightmost(PtrReplaced, &(*RootPtr)->RPtr);
		}
		else{ // najpravejsi uzol sa nasiel
			PtrReplaced->Key = (*RootPtr)->Key;
			PtrReplaced->BSTNodeCont = (*RootPtr)->BSTNodeCont;
			if((*RootPtr)->LPtr != NULL){ // najpravejsi uzol má ešte ľavý podstrom
				NodePtr = (*RootPtr);
...
**/
	...
	if(RootPtr != NULL){
		if((*RootPtr)->Key == K){ // nasiel sa uzol :)
			if((*RootPtr)->LPtr == NULL || (*RootPtr)->RPtr == NULL){ // jeden alebo ziadny potomkovia
				NodePtr = (*RootPtr);
			...
			else{ // uzol má dvoch potomkov
				ReplaceByRightmost((*RootPtr), &NodePtr);
			}
		}
		else{ // msi sa hľadať ďalej :(
			if((*RootPtr)->Key > K && (*RootPtr)->LPtr != NULL){
				BSTDelete (&(*RootPtr)->LPtr, K);
			}
			if((*RootPtr)->Key < K && (*RootPtr)->RPtr != NULL){
...
**/
	...
	if(IsRoot){
		if( NodePtr == NULL ){ // prázdny strom
			return -1;
		}
		// neprázdny strom
		else // koreň stromu
			if(NodePtr->RPtr != NULL){
				RTree = BSTHeight (NodePtr->RPtr, FALSE) + 1;
			}
			if(NodePtr->LPtr != NULL){
				LTree = BSTHeight (NodePtr->LPtr, FALSE) + 1;
			}
			else{ //koren nemá potomkov
				return 1;
			}
			return RTree > LTree ? RTree : LTree;
	}
	else{ // ostatné uzly
		if(NodePtr->RPtr != NULL){
			RTree = BSTHeight (NodePtr->RPtr, FALSE) + 1;
...
**/
	...
	if((*RootPtr) != NULL){
		...
		free((*RootPtr)); // uvolním uzol
		(*RootPtr) = NULL; // nulujem ukazatele
	}
